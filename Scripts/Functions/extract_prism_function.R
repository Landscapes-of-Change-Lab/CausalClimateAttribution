## ---------------------------
##
## Script name: Function to parse and extract prism data
##
...
## ---------------------------
##
## Notes: Requires downloaded prism tmax data
##   
##
## ---------------------------
...
# Function to parse dates from PRISM folder names
parse_prism_date <- function(filepath) {
  # Get the folder name
  ...
  # or PRISM_tmax_provisional_4kmM3_198209_bil)
  date_str <- str_extract(folder_name, "\\d{6}(?=_bil)")
  
  if(is.na(date_str)) {
    warning("Could not extract date from folder: ", folder_name)
    return(NULL)
  }
...
find_prism_files <- function(base_dir) {
  # List all subdirectories that match PRISM pattern
  all_dirs <- list.dirs(base_dir, full.names = TRUE, recursive = FALSE)
  prism_dirs <- all_dirs[grepl("PRISM_tmax_(?:stable|provisional)_4kmM3_\\d{6}_bil", basename(all_dirs))]
...
extract_prism_data <- function(base_dir, waypoints_sf) {
  cat("Starting PRISM data extraction...\n")
  
  # Find all PRISM files
  cat("Searching for PRISM files...\n")
  prism_files <- find_prism_files(base_dir)
  ...
  for(i in seq_along(prism_files)) {
    ...
    tryCatch({
      ...
      # Get date and stability information
      date_info <- parse_prism_date(file)
      ...
      if(!is.null(date_info)) {
        ...
        temp_df <- data.frame(
          ...
          tmax = extracted_values[,2],
          year = date_info$year,
          month = date_info$month,
          date = date_info$date,
          is_provisional = date_info$is_provisional,
          is_stable = date_info$is_stable,
