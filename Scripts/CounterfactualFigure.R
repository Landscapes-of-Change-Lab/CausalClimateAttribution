...
## counterfactual data
counter <- read_csv(here("Data", "extracted_counterfactual_data.csv")) %>% 
  rename(counter_tmax = tmax)

## observed prism data
prismdat <- read_csv(here("Data", "extracted_prism_data.csv")) %>% 
...
# 4. Moving Average detrended
window_size <- 10
splinedat$ma_trend <- rollmean(splinedat$tmax, k = window_size, fill = NA)
splinedat$ma_detrended <- splinedat$tmax - splinedat$ma_trend + mean(splinedat$tmax, na.rm = TRUE)

# Cleaning data
data_long <- splinedat %>%
  group_by(year) %>% 
  summarize(meant=mean(tmax), m.MA_trend = mean(ma_detrended), gamdtrend = mean(gam_detrended),
